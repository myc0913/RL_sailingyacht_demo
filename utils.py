...
class Actor(nn.Module):
	...
	def summary_forward(self, state):
		mu, _ = self.forward(state, deterministic=False, with_logprob=False)
		return mu
class Double_Q_Critic(nn.Module):
	def __init__(self, state_dim, action_dim, hid_shape_q1, hid_shape_q2):
		super(Double_Q_Critic, self).__init__()
		layers1 = [state_dim + action_dim] + list(hid_shape_q1) + [1]
		layers2 = [state_dim + action_dim] + list(hid_shape_q2) + [1]

		self.Q_1 = build_net(layers1, nn.ReLU, nn.Identity)
		self.Q_2 = build_net(layers2, nn.ReLU, nn.Identity)
