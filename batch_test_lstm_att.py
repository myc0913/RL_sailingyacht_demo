...
parser.add_argument('--max_e_steps', type=int, default=900, help='Maximum steps per episode')
...
BrifEnvName = ['saclstm_s14_0604_lstm1_att']
...
def run_single_test(agent, env, opt, test_id):
    ...
    # 初始化前一步的状态和动作
    prev_state = s.copy()
    prev_action = np.zeros(opt.action_dim)
    
    # 为每个测试分配唯一的环境ID
    env_id = f"test_env_{test_id}"
    agent.actor.reset_hidden_state(env_id)
    ...
    while not done and step < opt.max_e_steps:
        ...
        a = agent.select_action(prev_state, prev_action, s, deterministic=True, 
                               total_steps=None, env_id=env_id)
...
def main():
    ...
    stats_filename = f'{desktop_path}batch_test_stats_{timestamp}.json'
    with open(stats_filename, 'w', encoding='utf-8') as f:
        json.dump(stats, f, indent=2, ensure_ascii=False)
    print(f"统计结果已保存至: {stats_filename}")
    
    # 保存简要结果摘要
    summary_filename = f'{desktop_path}batch_test_summary_{timestamp}.txt'
